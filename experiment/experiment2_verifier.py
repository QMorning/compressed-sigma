...
for line in content:  
    ...
    if line.startswith('Sumcheck on F+G Verifier time') and current_experiment == "experiment 2-1":             
...
for line in content:  
    ...
    elif line.startswith('Sumcheck on F+G Verifier time') and current_experiment == "experiment 2-2":             
            time_data = extract_number(line)
...
for line in content:  
    ...
    elif line.startswith('Sumcheck on F+G Verifier time') and current_experiment == "experiment 2-3":             
            time_data = extract_number(line)
...
#plt.xticks(num_x[:len(num_x)//2], ['$2^2$', '$2^4$', '$2^6$', '$2^8$', '$2^{10}$'])
selected_ticks = [2, 4, 6, 8, 10]
plt.xticks(selected_ticks, ['$2^2$', '$2^4$', '$2^6$', '$2^8$', '$2^{10}$'])
#plt.xticks(num_x, ['$2^1$', '$2^2$', '$2^3$', '$2^4$', '$2^5$', '$2^6$', '$2^7$', '$2^8$', '$2^9$','$2^{10}$'], rotation=45)
plt.xlabel('m')
plt.ylabel('time (ms)')
plt.legend()
